//
//  AnimatedGridCardFlowLayout.swift
//  RHPokemonDemoApp
//
...
class AnimatedGridCardFlowLayout: UICollectionViewFlowLayout {
    ...
    func indexPathForCenteredVisibleCell() -> IndexPath? {
        guard let collectionView = collectionView else { return nil }
        
        // 計算視圖中心點相對於整個內容的座標
        let centerPoint = CGPoint(x: collectionView.contentOffset.x + collectionView.bounds.size.width / 2,
                                  y: collectionView.contentOffset.y + collectionView.bounds.size.height / 2)
        
        // 使用這個中心點找到對應的 indexPath
